    ...
    myRequest.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            submit_button.onclick = function () {
                ...
                moveAnswers(questionsCount);
                showExplanation(questionsCount, questionTitles, user_answers);
            };
        }
    };
    myRequest.open("POST", "souf.php", true);
    myRequest.send();
}
// getQuestions();
...
function showResults(count, right_answers) {
    let theResults;
    if (currentIndex == count) {
        quiz_app.remove();
...
function showStats(count, questionTitles, user_answers, comments) {
    if (currentIndex == count) {
        for (let i = 0; i < count; i++) {
            ...
            let questionNumText = document.createTextNode(
                "Question Number" + (i + 1)
            );
            ...
            /////////////////////////////
            getCorrectAnswers(count, i, stat);

            //create the p that contains the comment
            let comment = document.createElement("p");
            comment.className = "explanation";
            let commentText = document.createTextNode(comments[i].exp);
...
//transform users ansewers to json

function moveAnswers(count) {
    // if (currentIndex == count) {
    // const array = [1, 2, 3];
    ...
    //     }
    // }
    // xhr.send(`array=${encodeURIComponent(JSON.stringify(user_answers))}`);
    // }
    if (currentIndex == count) {
        $.get(
            "./controller.php",
            ...
            function (data) {
                showResults(count, data);
            }
        );
    }
}
function showExplanation(count, questionTitles, user_answers) {
    if (currentIndex == count) {
        $.post("./explanation.php", function (data) {
            // Parse the returned JSON object
            var comments = JSON.parse(data);
            // show the explanation
            showStats(count, questionTitles, user_answers, comments);
        });
    }
}

function getCorrectAnswers(count, i, stat) {
    if (currentIndex == count) {
        $.post("./correctAnswers.php", function (data) {
            // Parse the returned JSON object
            var correctAnswers = JSON.parse(data);
            // show the explanation
            showCorrect(correctAnswers, i, stat);
            showAnswers(user_answers, correctAnswers, i, stat);
        });
    }
}
function showCorrect(correctAnswers, i, stat) {
    // create the p that contains the correct answer
    let correctAnswer = document.createElement("p");
    correctAnswer.className = "correct-response";
    let correctAnswerText = document.createTextNode(correctAnswers[i].answer);
    correctAnswer.appendChild(correctAnswerText);
    stat.appendChild(correctAnswer);
}

function showAnswers(user_answers, correctAnswers, i, stat) {
    ...
    if (user_answers[i] == correctAnswers[i].answer) {
